...
function computeCurrentStreak(
  ...
  const yesterday = getLocalDate(offsetDate(new Date(today), -1));
  ...
  while (dateSet.has(cursor)) {
    streak++;
    cursor = getLocalDate(offsetDate(new Date(cursor), -1));
  }
...
function computeLongestStreak(sortedAscDates: string[]): number {
  ...
  for (let i = 1; i < sortedAscDates.length; i++) {
    const expected = getLocalDate(
      offsetDate(new Date(sortedAscDates[i - 1]), 1),
    );
...
export function computeMomentum(
  ...
  const thirtyDaysAgo = getLocalDate(offsetDate(new Date(today), -30));
